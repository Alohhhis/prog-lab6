...
class ServerMain(
    ...
    private val saver: Saver<LinkedHashMap<Int, Vehicle>> by inject()
    private val storage: Storage<LinkedHashMap<Int, Vehicle>, Int, Vehicle> by inject()
    private val serializer = FrameSerializer()
    private val logger = Logger.getLogger(ServerMain::class.java)
    var running = true
...
//            socketChannel.close()
    ...
    private fun clientRequest(request: Frame): Frame {
        return when (request.type) {
            ...
            FrameType.LIST_OF_COMMANDS_REQUEST -> {
                ...
                val commands = commandManager.commands.mapValues { it.value.getArgumentType() }.toMap()
    ...
    fun saveCollection() {
        val saver: Saver<LinkedHashMap<Int, Vehicle>> by inject()
